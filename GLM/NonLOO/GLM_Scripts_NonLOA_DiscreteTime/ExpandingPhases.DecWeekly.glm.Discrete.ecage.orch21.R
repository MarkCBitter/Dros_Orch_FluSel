...
for (tp in tps){
    ...
    #fwrite(res, paste0('df.glm.',as.character(vec[1]),'_', as.character(vec[length(vec)]),'.csv'), row.names = FALSE)
    df.glm = cbind(df.glm, res)
    save(df.glm, file = "./05_GLM/ExpandingPhases_DecWeekly_glm_Discrete.ecage.orch2021.RData")
    }